...
class OTGTabFragment: Fragment(), BackpressCallback, LoaderManager.LoaderCallbacks<Boolean> {
    ...
    private var mRoot = Constant.OTG_ROOT ?: "/"
    ...
    override fun onCreateView(
        ...
        mFileActionManager = FileActionManager(context!!, FileActionManager.FileActionServiceType.OTG, this)   //action manager
    ...
    class TabPagerAdapter(fragmentActivity: FragmentActivity, root: String): FragmentStateAdapter(fragmentActivity) {
        val Pager_Count = 5
        var allFilePage: OTGFragment
        var imagePage: OTGMediaFragment
        var musicPage: OTGMediaFragment
        var videoPage: OTGMediaFragment
        var docPage: OTGMediaFragment

        init{
            allFilePage = OTGFragment()
            imagePage = OTGMediaFragment(Constant.TYPE_IMAGE, root)
            musicPage = OTGMediaFragment(Constant.TYPE_MUSIC, root)
            videoPage = OTGMediaFragment(Constant.TYPE_VIDEO, root)
            docPage = OTGMediaFragment(Constant.TYPE_DOC, root)
        }
